import * as Plot from "../_npm/@observablehq/plot@0.6.17/ddfe0eea.js";
import * as aq from "../_npm/arquero@8.0.1/40fb4c55.js";
import * as d3 from "../_npm/d3@7.9.0/27911eeb.js";
import { color_inst, color_subcluster, grey_innosuisse, black_innosuisse, grey_background, grey_comment, color_zufrieden, colors_dark, color_instrument} from "./colors.79c45eba.js";
...
import {
  ...
  instrument_link
} from "./data.cfb39f11.js"

export const language = {value: "en", label: "English"}
...
export const plot_erhebung = Plot.plot({
  ...
  marks: [
    ...
    Plot.text(
      [
        { year: 3, text: "Survey 1:\nAt completion of funding" },
        { year: 6, text: "Survey 2:\n3 years after completion" }
      ],
      {
        x: "year",
        y: 1,
    ...
    // Annotate the phases
    Plot.text([{ year: 1.5, text: "Implementation \nof support offer" }], {
      x: "year",
      y: 1,
      text: "text",
      fontSize: 12,
...
export function draw_fin_plot(
  ...
  const data = aq
    ...
    .groupby("FA", "inst", "instrument_en", "type", "label_en", "monitoring")
    .rollup({
  ...
  return Plot.plot({
    ...
    x: {
      ...
      labelOffset: 45,
      label: displayXAxis
        ? "Approved funding in millions of Swiss francs per year (⌀ 2021-2023)"
        : null
    },
    ...
    marks: [
      ...
      Plot.barX(df, {
        x: "mean_funding",
        y: (d) => (d.monitoring === "Ja" ? `*${d.instrument_en}` : d.instrument_en),
      ...
      Plot.textX(df, {
        ...
        y: (d) => (d.monitoring === "Ja" ? `*${d.instrument_en}` : d.instrument_en),
        text: (d) =>
          d.instrument_en === "Start-up innovation projects (2023+)"
            ? `CHF ${d.mean_funding.toLocaleString("de-CH")} mn (2023)`
            : d.instrument_en === "BRIDGE Proof of Concept" ||
              d.instrument_en === "BRIDGE Discovery"
            ? `⌀ CHF ${d.mean_funding.toLocaleString("de-CH")} mn (The amount is doubled by the SNSF)`
            : `⌀ CHF ${d.mean_funding.toLocaleString("de-CH")} mn`,
        textAnchor: "start",
        dx: 5,
        sort: { y: "x", reverse: true }
      }),
...
export const n_subcluster = Plot.plot({
  ...
  x: {
    ...
    percent: true,
    label:
      "Share of approved innovation projects in % by subject area by support offer (⌀ 2021-2023)",
    ticks: []
  },
  y: {
    label: "",
...
export function drawMiniPlot(instrument, funding = true) {
  ...
  return Plot.plot({
    ...
    marginRight: 40,
    marginLeft: 0,
    y: funding ? { label: "CHF mn" } : { label: label, domain: instrument !== "Start-up Coaching" ? undefined : [0,400] },
    x: {
...
export const leverage = Plot.plot({
    ...
    marks: [
      ...
      Plot.text(
        [
          { x: 110, y: 105, text: "1 CHF funding" },
          { x: 350, y: 105, text: "4 CHF of value added" }
        ],
        {
          x: "x",
          y: "y",
...
export const kof_did_plot = Plot.plot({
  ...
  x: {
    ...
    label: "Year in relation to completion of the innovation project"
  },
  y: {
    domain: [-75, 75],
    ...
    labelOffset: 15,
    label:
      "Difference between companies with Innosuisse funding and the control group in %"
  },
  fy: {
    label: null,
  ...
  marks: [
      ...
      [
        {
          x: "t+4",
          y: 0.14,
          text: "↑ Companies with Innosuisse funding grow stronger"
        },
        {
          x: "t+4",
          y: -0.15,
          text: "↓ Control group companies grow stronger"
        }
      ],
      {
        x: "x",
    ...
    Plot.line(
      [
        { type_de: "Sales", year_text: "t", y: 0.21 },
        { type_de: "Sales", year_text: "t+4", y: 0.21 },
        { type_de: "Employment", year_text: "t", y: 0.18 },
        { type_de: "Employment", year_text: "t+4", y: 0.18 }
      ],
      {
        x: "year_text",
        y: "y",
...
export const su_vza = Plot.plot({
    ...
    x: {
      ...
      label:
        "⌀ Number of full-time employees after completing Core Coaching and 3 years later"
    },
    y: { axis: null },
    marks: [
      Plot.barX(
