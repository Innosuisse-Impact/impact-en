// Data import
import { FileAttachment } from "../_observablehq/stdlib.83ab9eb5.js";

export const daten_controlling = await FileAttachment({"name":"../data/daten_controlling.txt","mimeType":"text/plain","path":"../_file/data/daten_controlling.d8ca2b9f.txt","lastModified":1740557176536,"size":12244}, import.meta.url).tsv({ typed: true });

const df_subcluster = await FileAttachment({"name":"../data/daten_subcluster.txt","mimeType":"text/plain","path":"../_file/data/daten_subcluster.e69460e9.txt","lastModified":1740558792000,"size":3507}, import.meta.url).tsv({ typed: true });
...
export const instrument_link_hex = await FileAttachment({"name":"../data/instrument_link_hex.txt","mimeType":"text/plain","path":"../_file/data/instrument_link_hex.3e5dec9c.txt","lastModified":1740564491964,"size":1188}, import.meta.url).tsv({ typed: true } )
export const instrument_link = new Map(
  instrument_link_hex.map(item => [item.instrument, item.link])
)

export const df_waffle = await FileAttachment({"name":"../data/daten_waffle.txt","mimeType":"text/plain","path":"../_file/data/daten_waffle.068769dc.txt","lastModified":1740565153377,"size":19177}, import.meta.url).tsv({ typed: true });
...
export const kof_did = await FileAttachment({"name":"../data/daten_kof_did.txt","mimeType":"text/plain","path":"../_file/data/daten_kof_did.8a69cf01.txt","lastModified":1740488102611,"size":931}, import.meta.url)
  .tsv()
  .then((data) =>
    data.map((d) => {
      return {
...
const df_ziel_erfolg = await FileAttachment({"name":"../data/daten_zielerreichung.txt","mimeType":"text/plain","path":"../_file/data/daten_zielerreichung.559abc30.txt","lastModified":1740560035325,"size":17882}, import.meta.url).tsv({ typed: true });
...
export const df_ergebnisse = await FileAttachment({"name":"../data/daten_ergebnisse.txt","mimeType":"text/plain","path":"../_file/data/daten_ergebnisse.869f0158.txt","lastModified":1740488102610,"size":14637}, import.meta.url).tsv({ typed: true });
const innovationsart = await FileAttachment({"name":"../data/daten_innovationsart.txt","mimeType":"text/plain","path":"../_file/data/daten_innovationsart.500e2762.txt","lastModified":1740488102610,"size":28107}, import.meta.url).tsv({ typed: true });
  ...
  instrument_n:
    ...
    ? "Innovation projects with\nimplementation partner"
      ...
      ? "Innovation projects without\nimplementation partner"
        : d.instrument_en,
  type_en_n:
        d.type_en === "innovation de modèle d'entreprise"
          ? "innovation\nde modèle d'entreprise"
          : d.type_en === "innovation de produit"
        ? "innovation\nde produit"
        : d.type_en === "innovation en matière de services"
